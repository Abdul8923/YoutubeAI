  ...
  return (
    <div className="space-y-6">
      {questions.map((q, qIndex) => (
        <div key={qIndex} className="p-4 bg-slate-900/30 border border-slate-700 rounded-lg">
          ...
          <div className="space-y-2">
            {q.options.map((option, oIndex) => (
              ...
              >
                ...
                {isSubmitted && option === q.correctAnswer && <CheckCircleIcon className="h-5 w-5 text-green-400" />}
                {isSubmitted && selectedAnswers[qIndex] === option && option !== q.correctAnswer && <XCircleIcon className="h-5 w-5 text-red-400" />}
              </button>
            ))}
          </div>
        </div>
